...
### ติดตั้งบนเครื่อง Windows ที่ไม่มีอินเทอร์เน็ต (offline / air-gapped)

ถ้าเครื่องที่จะรันโปรแกรม **ออกเน็ตไม่ได้เลย** (และไม่มี Docker ด้วย) ยังติดตั้งแบบ isolated ได้ปกติ —
ไลบรารีทั้งหมดที่ `start.bat` ต้องใช้จะถูกลงใน virtual environment (`.venv`) ของโปรเจกต์นี้เท่านั้น
**ไม่แตะ/ไม่ขึ้นกับ Python ที่มีอยู่เดิมในเครื่อง** ไม่ว่าเครื่องนั้นจะมี Python อยู่แล้วหรือไม่ก็ตาม

สิ่งที่ต้องเตรียมจากเครื่องที่มีอินเทอร์เน็ต (แล้วก๊อปข้ามมาทาง LAN/แชร์โฟลเดอร์):

1. **ตัวติดตั้ง Python 3.12 สำหรับ Windows** — โหลดจาก
   [python.org/downloads/windows](https://www.python.org/downloads/windows/) (ใช้ตัว "Windows installer
   (64-bit)") บนเครื่องไหนก็ได้ที่ออกเน็ตได้ แล้วก๊อปไฟล์ `.exe` นั้นมาลงบนเครื่องออฟไลน์ ตอนติดตั้งอย่าลืมติ๊ก
   **"Add python.exe to PATH"**
2. **ชุดไลบรารีที่ดาวน์โหลดมาล่วงหน้า (offline wheel bundle)** — เตรียมไว้ให้แล้ว ขอรับไฟล์
   `collection-data-offline-wheels-win_amd64-py312.zip` จากผู้ที่ส่งโปรเจกต์นี้ให้คุณ แล้วแตกไฟล์ zip นั้นลง
   ในโฟลเดอร์ `vendor\wheels\` ของโปรเจกต์นี้ (จะมีไฟล์ `.whl` ประมาณ 36 ไฟล์ รวม ~13MB) — โฟลเดอร์นี้มีอยู่แล้ว
   ในโปรเจกต์ (ว่างเปล่า มีแค่ `.gitkeep`) แค่เอาไฟล์ `.whl` ไปวางแทนที่

จากนั้นดับเบิลคลิก `start.bat` ได้ตามปกติ — สคริปต์จะ**ตรวจพบไฟล์ `.whl` ใน `vendor\wheels\` อัตโนมัติ**
แล้วสลับไปติดตั้งจากโฟลเดอร์นั้นแทนการโหลดจาก pypi.org โดยไม่ต้องตั้งค่าอะไรเพิ่ม (และจะไม่พยายามต่อเน็ตเลย
ในขั้นตอนติดตั้ง) ถ้าไม่พบไฟล์ `.whl` ในโฟลเดอร์นั้น สคริปต์จะ fallback กลับไปโหลดจากอินเทอร์เน็ตตามปกติ
(เผื่อกรณีใช้ repo เดียวกันบนเครื่องอื่นที่ต่อเน็ตได้)

> ชุด wheel ที่เตรียมไว้ครอบคลุมไลบรารีที่จำเป็นทั้งหมดสำหรับรันบน **Windows 64-bit + Python 3.12** เท่านั้น
> ถ้าเครื่องออฟไลน์ใช้ Python เวอร์ชันอื่น หรือเป็น 32-bit ต้องขอชุด wheel ใหม่ที่ตรงเวอร์ชัน (แจ้งผู้ที่ดูแล
> โปรเจกต์นี้เพื่อสร้างชุดใหม่ให้ตรงกับเวอร์ชัน Python บนเครื่องนั้น)
